...
void test_sleep(){
    star::IOManager ioManager(1);
    ioManager.submit([](){
        STAR_LOG_INFO(g_logger) << "2 sec start";
        usleep(2000'000);
        STAR_LOG_INFO(g_logger) << "2 sec end";
    });
    ioManager.submit([](){
        STAR_LOG_INFO(g_logger) << "3 sec start";
        sleep(3);
        STAR_LOG_INFO(g_logger) << "3 sec end";
    });
    STAR_LOG_INFO(g_logger) << "test sleep";
}
std::set<int> fds;
...
void test_connect(){
    ...
    listen(sock,5);
    STAR_LOG_INFO(g_logger) << "begin accept";
    int len = sizeof(addr);
    int fd = 0;
    while(( fd = accept(sock, (sockaddr*)&addr, reinterpret_cast<socklen_t *>(&len)))){
        fds.insert(fd);
        STAR_LOG_INFO(g_logger) << "accept rt=" << fd << " errno=" << errno;
        sendAll(fd,"New User, welcome him\n", 21);
        star::IOManager::GetThis()->submit([fd]{
            char *buff = new char[4096];
            while(1){
...
void test_echo(){
    ...
    STAR_LOG_INFO(g_logger) << "accept rt=" << rt << " errno=" << errno;
...
void test_http_client(){
    ...
    STAR_LOG_INFO(g_logger) << "begin connect";
    int rt = connect(sock, (const sockaddr*)&addr, sizeof(addr));
    STAR_LOG_INFO(g_logger) << "connect rt=" << rt << " errno=" << errno;
    ...
    STAR_LOG_INFO(g_logger) << "send rt=" << rt << " errno=" << errno;
    ...
    STAR_LOG_INFO(g_logger) << "recv rt=" << rt << " errno=" << errno;
