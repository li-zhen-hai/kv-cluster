...
void Scheduler::start() {
    ...
    m_stop = false;
    STAR_ASSERT(m_threads.empty());
    m_threads.resize(m_threadCount);
    m_threadIds.resize(m_threadCount);
    for(size_t i = 0; i < m_threadCount; ++i){
        m_threads[i].reset(new star::Thread(m_name + '_' + std::to_string(i),[this]{
            this->run();
        }));
...
void Scheduler::notify() {
    STAR_LOG_INFO(g_logger) << "notify";
}

void Scheduler::wait() {
    //m_sem.wait();
    STAR_LOG_INFO(g_logger) << "idle";
    while(!stopping()) {
        star::Fiber::YieldToHold();
    }
    STAR_LOG_DEBUG(g_logger) << "idle fiber exit";
}
