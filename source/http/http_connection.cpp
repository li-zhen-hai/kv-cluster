...
#include "star/http/http_connection.h"
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    while (!parser->isFinished()) {
        ...
        if (parser->hasError() || nparse == 0) {
            STAR_LOG_DEBUG(g_logger) << parser->hasError();
            close();
            return nullptr;
        }
        offset = len - nparse;
    ...
    if (parser->isChunked()) {
        do {
            ...
            //STAR_LOG_INFO(g_logger) << data;
            if (len <= 0) {
                STAR_LOG_DEBUG(g_logger) << "parse chunk error, code=" << parser->hasError();
                close();
                return nullptr;
            }
            len += offset;
...
HttpConnection::~HttpConnection() {
    STAR_LOG_DEBUG(g_logger) << "~HttpConnection";
}
...
HttpConnectionPool::Create(const std::string &uri, const std::string &vhost, uint32_t max_size,
    ...
    if(turi) {
        STAR_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = star::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            STAR_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        ...
        if(!sock) {
            STAR_LOG_ERROR(g_logger) << "create sock fail: " << addr->toString();
            return nullptr;
        }
        if(!sock->connect(addr)) {
            STAR_LOG_ERROR(g_logger) << "sock connect fail: " << addr->toString();
            return nullptr;
        }
...
void HttpConnectionPool::RelesePtr(HttpConnection *ptr, HttpConnectionPool *pool) {
    ++ptr->m_request;
    if (!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) < star::GetCurrentMS())
