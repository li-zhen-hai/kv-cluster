...
static star::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
        star::Config::Lookup("http.request.buffer_size"
                ,(uint64_t)(4 * 1024), "http request buffer size");

static star::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
        star::Config::Lookup("http.request.max_body_size"
                ,(uint64_t)(64 * 1024 * 1024), "http request max body size");

static star::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
        star::Config::Lookup("http.response.buffer_size"
                ,(uint64_t)(4 * 1024), "http response buffer size");

static star::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
        star::Config::Lookup("http.response.max_body_size"
...
void HttpRequestParser::on_request_method(const std::string& str) {
    ...
    if(method == HttpMethod::INVALID_METHOD) {
        STAR_LOG_WARN(g_logger) << "invalid http request method: " << str;
...
void HttpRequestParser::on_request_version(const std::string& str) {
    ...
    if(str == "1.1") {
        ...
        STAR_LOG_WARN(g_logger) << "invalid http request version: " << str;
...
void HttpRequestParser::on_request_header(const std::string& key, const std::string& val) {
    if(key.empty()) {
        STAR_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
void HttpResponseParser::on_response_version(const std::string &str) {
    ...
    if(str == "1.1") {
        ...
        STAR_LOG_WARN(g_logger) << "invalid http response version: "
                                << str;
        setError(INVALID_VERSION);
        return;
    }
