...
void IOManager::FdContext::triggerEvent(IOManager::Event event) {
    //STAR_ASSERT2(events & event,std::to_string(event)+" & "+std::to_string(events)+" = "+std::to_string(events & event));
    if(!(events & event)){
        STAR_LOG_ERROR(STAR_LOG_ROOT()) << "ASSERTION: " << (events & event)
...
bool IOManager::addEvent(int fd, IOManager::Event event, std::function<void()> cb) {
    //STAR_LOG_DEBUG(g_logger) << "addEvent() : fd=" << fd << " event=" << (event==1? "read" : "write");
    ...
    if(fdContext->events & event){
        STAR_LOG_ERROR(g_logger) << "fd=" << fd << " addEvent fail, event already register. "
                << "event=" << event << " FdContext->event=" << fdContext->events;
        STAR_ASSERT(!(fdContext->events & event));
    }
    ...
    if(rt){
        STAR_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << op << ", " << fd << ", "
...
bool IOManager::cancelAllEvent(int fd) {
    ...
    STAR_ASSERT(fdContext->events == 0);
    return true;
}
...
void IOManager::wait() {
    //STAR_LOG_DEBUG(g_logger) << "wait for event";
    ...
    while (true){
        ...
        if(stopping(next_timeout)){
            STAR_LOG_INFO(g_logger) << "name=" << getName()
                                     << " idle stopping exit";
            return;
        }
        int rt = 0;
        ...
        for (int i = 0; i < rt; ++i) {
            ...
            if(res){
                STAR_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << op << ", " << fdContext->fd << ", "
