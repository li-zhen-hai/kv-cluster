...
bool Socket::getOption(int level, int option, void* result, size_t* len) {
    ...
    if(rt){
        STAR_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, void* result, size_t len){
    ...
    if(rt){
        STAR_LOG_ERROR(g_logger) << "setOption sock=" << m_sock
                << " level=" << level << " option=" << option
                << " errno=" << errno << " errorstr=" << strerror(errno);
        return false;
    }
...
void Socket::newSocket() {
    ...
    if(STAR_LIKELY(m_sock != -1)){
        initSocket();
    } else {
        STAR_LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", " << m_protocol << ") "
...
Socket::ptr Socket::accept() {
    ...
    if(newsock < 0){
        STAR_LOG_DEBUG(g_logger) << "accept(" << m_sock << ") "
                << "errno=" << errno << "errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr address) {
    ...
    if(STAR_UNLIKELY(m_family != address->getFamily())){
        STAR_LOG_ERROR(g_logger) << "bind sock.family(" << m_family << ") address.family(" << address->getFamily() << ")"
            << " not equal, address=" << address->toString();
        return false;
    }

    if(::bind(m_sock, address->getAddr(), address->getAddrLen())){
        STAR_LOG_ERROR(g_logger) << "bind address=" << address->toString() << " error, errno="
...
bool Socket::connect(const Address::ptr address, uint64_t timeout_ms) {
    ...
    if(STAR_UNLIKELY(m_family != address->getFamily())){
        STAR_LOG_ERROR(g_logger) << "connect sock.family(" << m_family << ") address.family(" << address->getFamily() << ")"
                                 << " not equal, address=" << address->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1){
        if(::connect(m_sock, address->getAddr(), address->getAddrLen())){
            STAR_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << address->toString() << ") error, errno="
                                     << errno << " errstr=" << strerror(errno);
            close();
            return false;
        }
        ...
        if(::connect_with_timeout(m_sock, address->getAddr(), address->getAddrLen(), timeout_ms)){
            STAR_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << address->toString() << ") error, timeout="
...
bool Socket::listen(int backlog){
    if(STAR_UNLIKELY(!isValid())){
        STAR_LOG_ERROR(g_logger) << "listen error, sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)){
        STAR_LOG_ERROR(g_logger) << "listen error, errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
    return true;
}
