...
bool TcpServer::bind(const std::vector<Address::ptr> &addrs, std::vector<Address::ptr> &fail) {
    for(Address::ptr addr : addrs){
        ...
        if(!sock->bind(addr)) {
            STAR_LOG_ERROR(g_logger)  << "bind fail errno="
        ...
        if(!sock->listen()) {
            STAR_LOG_ERROR(g_logger)  << "listen fail errno="
                                      << errno << " errstr=" << strerror(errno)
                                      << " addr=[" << addr->toString() << "]";
            fail.push_back(addr);
            continue;
    ...
    for(auto& sock : m_listens) {
        STAR_LOG_INFO(g_logger) << "Server name=" << m_name << " bind:" << sock->toString() << " success";
    }
...
bool TcpServer::start() {
    ...
    for(auto& sock : m_listens) {
        m_acceptWorker->submit([self, sock] {
            //STAR_LOG_DEBUG(g_logger) << "acceptWorker->submit";
            self->startAccept(sock);
        });
    }
    STAR_LOG_DEBUG(g_logger) << "TcpServer::start()";
...
void TcpServer::startAccept(Socket::ptr sock) {
    ...
    while(!isStop()) {
        ...
        if(client) {
            ...
            STAR_LOG_ERROR(g_logger) << "accept fail, errno=" << errno << " errstr=" << strerror(errno);
        }
    }
}

void TcpServer::handleClient(Socket::ptr client) {
    STAR_LOG_INFO(g_logger) << "handleClient: " << client->toString();
}
