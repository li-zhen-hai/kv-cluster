...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(error) {
        STAR_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>>& result,int family) {
    ...
    if(error) {
        STAR_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
    ...
    try {
        ...
        STAR_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char *host_name, uint16_t port) {
    ...
    if(error) {
        STAR_LOG_DEBUG(g_logger) << "IPAddress::Create(" << host_name
                                  << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char *ip, uint16_t port) {
    ...
    if(result <= 0) {
        STAR_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << ip << ", "
...
IPv6Address::ptr IPv6Address::Create(const char *ip, uint16_t port) {
    ...
    if(result <= 0) {
        STAR_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << ip << ", "
                                 << port << ") rt=" << result << " errno=" << errno
                                 << " errstr=" << strerror(errno);
        return nullptr;
    }
