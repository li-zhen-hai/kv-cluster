...
template<>
class LaxicalCast<YAML::Node,std::string>{
public:
...
template<>
class LaxicalCast<std::string,YAML::Node>{
public:
    YAML::Node operator()(const std::string s){
        return YAML::Node(s);
    }
};
...
public:
    ...
    std::string toString() override {
        try {
            ...
            STAR_LOG_ERROR(STAR_LOG_ROOT())<<"ConfigVar::toString() exception "
    ...
    bool fromString(const std::string& str) override {
        try {
            ...
            STAR_LOG_ERROR(STAR_LOG_ROOT())<<"ConfigVar::fromString() exception "
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,const T& value,const std::string& description){
        ...
        if(it != GetDatas().end()){
            ...
            if(tmp){
                STAR_LOG_INFO(STAR_LOG_ROOT())<<"lookup name="<<name<<" already exist";
                return tmp;
            }else{
                STAR_LOG_ERROR(STAR_LOG_ROOT())<<"lookup name="<<name<<" already exist but type not "
        ...
        if(name.find_first_not_of("abcdefghijklmnopqrstuvwxyz0123456789._")
                != std::string::npos){
            STAR_LOG_ERROR(STAR_LOG_ROOT())<<"lookup invalid name "<<name;
