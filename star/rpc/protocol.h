...
public:
    ...
    enum class MsgType : uint8_t {
        ...
        // RPC_FILE_MONITOR_REQUEST,            // 订阅监控文件
        // RPC_FILE_MONITOR_RESPONSE            // 发布文件变更事件

        //RAFT_APPENDLOGENTRY_REQUEST,         // 添加日志
        //RAFT_APPENDLOGENTRY_RESPONSE,

        //RAFT_REQUESTVOTE_REQUEST,            // 投票
        //RAFT_REQUESTVOTE_RESPONSE
    };

    static Protocol::ptr Create(MsgType type, const std::string& content, uint32_t id = 0,int version = 0) {
    ...
    static Protocol::ptr HeartBeat(int version=0) {
        static Protocol::ptr Heartbeat = Protocol::Create(Protocol::MsgType::HEARTBEAT_PACKET, "",0,version);
        return Heartbeat;
    }
