#ifndef STAR_RPC_SERVER_H
#define STAR_RPC_SERVER_H
#include <functional>
#include <memory>
#include <exception>
#include <stdexcept>
...
protected:
    ...
    void proxy(F fun, Serializer serializer, const std::string& arg) {
        ...
        try{
            ...
            val.setCode(star::rpc::RPC_SUCCESS);
            val.setVal(rt);
        }catch(std::exception& e){
            val.setMsg(e.what());
            val.setCode(star::rpc::RPC_FAIL);
        }catch(...){
            val.setCode(star::rpc::RPC_FAIL);
        }
        serializer << val;
