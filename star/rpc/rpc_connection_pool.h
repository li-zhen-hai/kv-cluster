...
#ifndef STAR_RPC_CONNECTION_POOL_H
#define STAR_RPC_CONNECTION_POOL_H
#include "star/traits.h"
#include "star/sync.h"
...
public:
    ...
    RpcConnectionPool();
    RpcConnectionPool(uint64_t timeout_ms);
    ~RpcConnectionPool();
    /**
     * @brief 连接 RPC 服务中心
     ...
     *          ServerLoad 服务负载
     */
    void setRouteStrategy(std::string r) { routeStrategy = r; }
     ...
     *         ServerLoad 服务负载
     */
    std::string getRouteStrategy() { return routeStrategy; }
    ...
    Result<R> call(const std::string& name, Params... ps) {
        ...
        Result<R> result;
        if (conn != m_conns.end()) {
            auto tmp = conn->second;
            lock.unlock();
            result = tmp->template call<R>(name, ps...);
        ...
        RouteStrategy<std::string>::ptr strategy =
                RouteEngine<std::string>::queryStrategy(routeStrategy);
        
        std::string fun_name = name;
        if(strategy == RouteEngine<std::string>::queryStrategy(Strategy::ServerLoad))
            fun_name = fun_name + "+";
    ...
    void subscribe(const std::string& key, Func func) {
        {
            ...
            if (it != m_subHandle.end()) {
                STAR_ASSERT2(false, "duplicated subscribe");
                return;
            }
...
private:
    ...
    // 服务名和服务地址的连接池
    std::unordered_map<std::string, RpcClient::ptr> m_conns;
    // 服务中心连接
...
#endif //STAR_RPC_CONNECTION_POOL_H
