...
#ifndef STAR_ROUTE_STRATEGY_H
#define STAR_ROUTE_STRATEGY_H
...
namespace impl {
     ...
     */
    template<class T>
    class SererLoadRouteStrategyImpl : public RouteStrategy<T> {
    public:
...
public:
    static typename RouteStrategy<T>::ptr queryStrategy(Strategy routeStrategyEnum) {
        switch (routeStrategyEnum){
            ...
            case Strategy::ServerLoad:
                return std::make_shared<impl::SererLoadRouteStrategyImpl<T>>();
            default:
                return s_randomRouteStrategy ;
        }
    }

    static typename RouteStrategy<T>::ptr queryStrategy(std::string routeStrategy) {
        if(routeStrategy == "Random")
            return s_randomRouteStrategy;
        if(routeStrategy == "Polling")
            return std::make_shared<impl::PollingRouteStrategyImpl<T>>();
        if(routeStrategy == "HashIP")
            return s_hashIPRouteStrategy;
        if(routeStrategy == "ServerLoad")
            return std::make_shared<impl::SererLoadRouteStrategyImpl<T>>();
        return s_randomRouteStrategy;
    }
private:
